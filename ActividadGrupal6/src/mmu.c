...
paddr_t mmu_init_kernel_dir(void) {
  ...
  kpd[0] = (pd_entry_t){
      .attrs = 0x11,// COMPLETAR: atributos para el directorio de paginas
  ...
  for (size_t i = 0; i <= VIRT_PAGE_TABLE(identity_mapping_end); i++) {
    kpt[i] = (pt_entry_t){
        .attrs = 0x11,// COMPLETAR: atributos para la tabla de paginas
...
void mmu_map_page(uint32_t cr3, vaddr_t virt, paddr_t phy, uint32_t attrs) {
  ...
  if ((pd[pdi].attrs & MMU_P) == 0) {
    ...
    pd[pdi].attrs |=  1; // COMPLETAR: que atributo debemos cambiar de forma explicita?
  }
  pd[pdi].attrs |= attrs;
...
paddr_t mmu_init_task_dir(paddr_t phy_start) {
  ...
  mmu_map_page(cr3, TASK_CODE_VIRTUAL + PAGE_SIZE * 2, stack, MMU_P | MMU_W | MMU_U);
